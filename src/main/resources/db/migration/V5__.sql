INSERT INTO server_setting_items (setting_key, clazz, setting_value)
    ...
    FROM dual
    WHERE NOT EXISTS (SELECT 1 FROM server_setting_items WHERE setting_key = 'grading.multipleChoicesQuestionsCheckingStrategy');

INSERT INTO server_setting_items (setting_key, clazz, setting_value)
    SELECT 'grading.enableLosePoints', 'java.lang.Boolean', 'false'
    FROM dual
    WHERE NOT EXISTS (SELECT 1 FROM server_setting_items WHERE setting_key = 'grading.enableLosePoints');
