        ...
        <div style="flex:1">
            ...
            <c:forEach var="choice" items="${question.getChoices()}">
                <div class="questionChoice"
                     <c:if test="${choice.correct}">correct</c:if> rounded>${fn:escapeXml(choice.getContent())}</div>
            </c:forEach>
                </div>
            </div>
        </div>
        ...
        <div style="display: flex;flex-direction: row;flex-wrap: wrap">
            ...
            <c:choose>
                <c:when test="${inUser_2}">
                    <button rounded highlight style="padding: 4px 6px;margin: 2px" onclick="switchToPageWithAnimate('server',2,false,function () {md5ToQuestionFormDataMap = new Map();editQuestion('${question.md5}')})">编辑</button>
                    <button rounded highlight style="padding: 4px 6px;margin: 2px" onclick="deleteQuestion('${question.md5}')">删除</button>
                </c:when>
                <c:otherwise>
                    <button rounded highlight style="padding: 4px 6px;margin: 2px" onclick="md5ToQuestionFormDataMap = new Map();editQuestion('${question.md5}')">编辑</button>
                    <button rounded highlight style="padding: 4px 6px;margin: 2px" onclick="deleteQuestion('${question.md5}')">删除</button>
                </c:otherwise>
            </c:choose>
        </div>
