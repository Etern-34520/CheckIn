...
const check = (questionInfo) => {
    questionInfo.errors = [];
    questionInfo.warnings = [];
    let question = questionInfo.question;
    if (question.content === undefined || question.content === "") {
        questionInfo.errors.push("题目内容为空");
    }
    if (question.partitionIds.length === 0) {
        questionInfo.errors.push("未选择分区");
    }
    if (question.choices.find((choice) => choice.correct) === undefined) {
        questionInfo.errors.push("无正确答案");
    }
    if (question.choices.find((choice) => choice && (choice.content === undefined || choice.content === "")) !== undefined) {
        questionInfo.errors.push("存在空选项");
    }
    if (question.authorQQ === undefined) {
        questionInfo.warnings.push("无作者");
    }
    questionInfo.showError = questionInfo.errors.length > 0;
    questionInfo.showWarning = false;
    // question.error = errors.length > 0;
    // question.warning = warnings.value.length > 0;
}
        ...
        <el-scrollbar v-else-if="ready">
            ...
            <div style="display: flex;flex-direction: column;padding: 8px">
                <transition-group name="alert">
                    <el-alert v-for="error of questionInfo.errors" :key="'error'+error" :title="error" type="error"
                              :closable="false"></el-alert>
                    <el-alert v-for="warning of questionInfo.warnings" :key="'warning'+warning" :title="warning"
                              type="warning"
                              :closable="false"></el-alert>
                </transition-group>
                ...
                <div style="display: flex;flex-direction: row">
                    ...
                    <div style="flex-grow:1;width: 60px;margin-left: 2px;display: flex;flex-direction: column">
                        ...
                        <div class="panel-1" style="padding: 2px">
                        ...
                        <transition-group :name="dragging ? null:'choice'">
                            <div class="choicePanel" v-for="(choice,$index) of questionInfo.question.choices"
                                ...
                                <transition name="delete-choice-button">
                                    <el-button class="remove-choice-button" v-show="questionInfo.question.choices.length>2" text @click="removeChoice($index)">
