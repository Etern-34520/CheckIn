...
const createQuestion = () => {
    questionInfo.value.question.questions.push({
        id: randomUUID(),
        content: "",
        enabled: false,
        partitionIds: null,
        ...
        <transition name="page-content" mode="out-in">
            ...
            <el-scrollbar v-else-if="ready">
                <div style="display: flex;flex-direction: column;padding: 8px">
                    ...
                    <div v-else-if="questionInfo.type === 'QuestionGroup'"
                         style="display: flex;flex-direction: column;overflow: hidden">
                        <VueDraggable
                            ref="draggable"
                            v-model="questionInfo.question.questions"
                            :animation="150"
                            ghostClass="ghost"
                            @start="onStartDrag"
                            @end="onEndDrag"
                        >
                            <transition-group :name="dragging ? null:'drag'">
                                <collapse v-for="question of questionInfo.question.questions" :key="question.id">
                                    <template #title>
                                        <div style="display: flex;flex-direction: row">
                                            <div class="handle" style="cursor: grab">
                                                <HarmonyOSIcon_Handle/>
                                            </div>
                                            <el-text>{{ question.content }}</el-text>
                                        </div>
                                    </template>
                                    <template #content>
                                        {{ question }}
                        ...
                        <el-button @click="createQuestion" size="large" text>
                            <HarmonyOSIcon_Plus :size="24"/>
                        </el-button>
                    </div>
                </div>
